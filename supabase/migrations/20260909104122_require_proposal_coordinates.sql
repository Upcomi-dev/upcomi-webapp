...
begin
  ...
  latitude_value := nullif(p_event->>'latitude', '')::double precision;
  longitude_value := nullif(p_event->>'longitude', '')::double precision;
  ...
  if p_status = 'approved' and latitude_value is null then
    raise exception 'Coordinates are required to publish an event';
  end if;
  ...
  set "nomEvent" = p_event->>'name',
      ...
      verifie = case
        ...
        when p_status in ('pending', 'rejected') then false
        else verifie
      end
  where id = p_event_id;
